...
        if (el.name) {
            ...
            if (el.required && value === '') {
                // Pour une meilleure expérience utilisateur, le navigateur gère la plupart des validations 'required'
                // mais ce test est maintenu pour une vérification JS côté client
                missingField = el; // Stocke l'élément manquant
            }
...
    drawLineField('Niveau d’études', 'niveau_etudes');
    drawLineField('Faculté / Domaine d\'études', 'faculte_etudes'); // Nouveau champ
...
    drawLineField('Nationalité du Gérant', 'nat_gerant');
    ...
    drawLineField('Genre du Gérant', 'genre_gerant'); // Nouveau champ
    drawLineField('Tranche d\'âge du Gérant', 'tranche_age_gerant'); // Nouveau champ
...
    // Permet au navigateur de gérer la validation HTML5 native (champs required)
    if (!document.querySelector('.a4').checkValidity()) {
        vibrate();
        // Le navigateur affiche déjà le message d'erreur natif pour le premier champ manquant.
        // On ne fait rien de plus ici pour ne pas interférer.
        return;
    }
...
    try {
        // Collecte les données (la fonction gère maintenant l'insertion de "Non précisé" si vide)
